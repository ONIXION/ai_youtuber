...
class ManagerFormat(BaseModel):
    feedback: str = Field(
        ..., description="Vtuberに対するフィードバック．scoreが2以下の場合に記述．"
    )
    score: int = Field(
        ..., ge=0, le=9, description="Vtuberの発言に対する評価．0から9の10段階．"
    )
...
async def web_search(input: Annotated[str, "what to search for"]) -> str:
    ...
    move_resize(x=0, y=0, width=1600, height=1100)  # 要調整
...
class AItuber:
    def __init__(self, port: int = 5000):
        ...
        self.setting_vr = self.create_vector_retriever(
            top_k=1, path="./chroma-db-setting"
        )
        self.memory_vr = self.create_vector_retriever(
...
"""
        ...
        self.manager_model = manager_prompt | gemini_flash.with_structured_output(
            ManagerFormat
        )
        # state graphを作成
    ...
    def call_talk_model(self, state: MessagesState):
        ...
        self.server.send_message_to_all(
            response.reply, response.action, response.emotion
        )
...
async def run_aituber(port: int = 5000):
    ...
    agent_input = TalkInput(
        name="初期設定", input="Nvidiaの株価について調べて"
    ).model_dump_json()
